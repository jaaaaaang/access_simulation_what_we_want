...
| **UI-1.7** | **RAPA Key 부분검색 (RAPA Key Quick Search)** *(NEW)* | 탑바 중앙 (RAPA Key 드롭다운 좌측) | RAPA Key 일부 문자열 또는 단지명으로 실시간 검색·조회. 결과 목록에 `temps` 캐시 존재/DB 교정 여부 배지(`캐시 N동` / `교정됨 N동`) 표시. Enter 시 최상위 결과를 즉시 조회, 항목 클릭 시 해당 대상으로 전환 |
...
| **UI-2.9** | **DB 저장 (원본 반영, DB Save)** *(NEW)* | 사이드바 · 도구(Tools) 패널 | 지우개 등으로 교정한 건물 폴리곤을 원본 소스 JSON(`public/temps/<rapaKey>.json`)에 되돌려 쓰는 저장. **Save Topology**(현재 세션 스냅샷을 브라우저로 export)와 달리 **마스터 데이터 자체를 교정**하며, 이후 같은 RAPA Key 재조회 및 LOS 배치 파이프라인에도 그대로 반영됨. 저장 전 유지/수정/추가/삭제 건물 수를 확인 다이얼로그로 표시하고, 최초 저장 시 원본을 서버가 1회 자동 백업. RAPA Key로 불러온 대상에서만 활성화되며, 서버(`npm run dev`) 구동 중에만 동작 |
| **UI-2.10** | **원본으로 복원 (Restore from Backup)** *(NEW)* | 사이드바 · 도구(Tools) 패널 | DB 저장으로 교정된 내용을 버리고 최초 백업 원본으로 되돌림. 복원 후에는 대상을 다시 조회해야 화면에 반영됨 |
